...
module.exports = {
  ...
  writeHtmlFileFromTemplate (req, res, next) {
    debug('write html file')
    const htmlTemplate = mustache.parseTemplate('./templates/invoice.mustache', {"name": "gitu"}) 
    fs.writeFile(`${req.temp.name}/html.html`, htmlTemplate, err => {
      if (err) res.status(500).send(err)
